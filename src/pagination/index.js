...
(function(factory) {
    ...
    Page.prototype.view = function() {
        ...
        if (conf.skip) {
            (function() {
                var skip_str = '<span class="laypage_total"><label>&#x5230;&#x7B2C;</label>' +
                    '<input type="number" min="1" onkeyup="this.value=this.value.replace(/\\D/, \'\');" class="laypage_skip"><label>&#x9875;</label>' +
    ...
    Page.prototype.jump = function(elem) {
        ...
        for (var i = 0, len = childs.length; i < len; i++) {
            if (childs[i].nodeName.toLowerCase() === 'a') {
                Page.on(childs[i], 'click', function(e) {
